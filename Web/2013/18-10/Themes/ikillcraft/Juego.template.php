...
$form_query = "SELECT DISTINCT categoria FROM juegos ORDER BY categoria DESC";
...
if(empty($_GET['cat'])) {
 while($rs2=mysql_fetch_array($result2))  {
    ...
    echo '<tr><td class="txt_desc" bgcolor="#FFFFFF"><img src="'.$rs2['miniatura'].'" width="104" height="70" align="left"><span style="position:relative;left:10px;"><span class="titulos"><a href="http://'.$_SERVER['SERVER_NAME'].'/test.php?id='.$rs2['id'].'">'.$rs2['nombre'].'</a></span><br>'.$rs2['descripcion'].'</span></td></tr>'; 

    if($int<$total){echo '<tr><td><hr width="98%" color="#000000" size="2"></td></tr>';}else{echo '<br>';}

 }

 if(mysql_num_rows($result2)==0){echo '<tr><td><center>No hay juegos.</center></td></tr>';}
...
while($rs=mysql_fetch_array($result)) 
  { 
    ...
    echo '<tr><td class="txt_desc" bgcolor="#FFFFFF"><img src="'.$rs['miniatura'].'" width="104" height="70" align="left"><span style="position:relative;left:10px;"><span class="titulos"><a href="http://'.$_SERVER['SERVER_NAME'].'/test.php?id='.$rs['id'].'">'.$rs['nombre'].'</a></span><br>'.$rs['descripcion'].'</span></td></tr>'; 
  
    if($int<$total){echo '<tr><td><hr width="98%" color="#000000" size="2"></td></tr>';}else{echo '<br>';}

  }

  if(mysql_num_rows($result)==0){echo '<tr><td><center>No hay juegos en esta categor&iacute;a.</center></td></tr>';}
...
/*while($row = mysql_fetch_assoc($form_result))
 {
 	echo "<option value=".sanear_string(strtolower($row[categoria])).">$row[categoria]</option>";
 }*/
