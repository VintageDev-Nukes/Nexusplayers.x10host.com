...
function darInsignia($id_del_usuario, $id_de_la_insignia) {
	...
	//$enlace = mysql_connect ($db_server, $db_user, $db_passwd);
	//if (!$enlace) {
    //die('No se pudo conectar: ' . mysql_error());
	//}
    //$bd_seleccionada = mysql_select_db ($db_name, $enlace);
    //if (!$bd_seleccionada) {
    //die ('No se pudo seleccionar correctamente la Base de Datos: ' . mysql_error());
	//}
    $query = mysql_query('SELECT badges FROM $db_prefixmembers WHERE id_member='.(int)$id_del_usuario);
	...
	if(is_array(mysql_fetch_array($query))) {
		...
		$id_array = array('badge_id' => $id_del_usuario);
