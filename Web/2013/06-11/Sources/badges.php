...
function Check_Badges()
{
	...
	global $context, $scripturl, $txt, $smcFunc, $user_info, $db_prefix;
		...
		$members_table = mysql_query("SELECT * FROM {$db_prefix}members WHERE id_member =".(int)$user_id);
		while ($mem = mysql_fetch_assoc($members_table)) {
			$good_karma = $mem['karma_good'];
        	$bad_karma = $mem['karma_bad'];
        	$online_time = $mem['total_time_logged_in'] / 86400;
        	$registered_time = $mem['date_registered'];
        	$money = $mem['money'];
        	$bank_money = $mem['moneyBank'];
		}
        $days_del_foro = floor(abs((time() - 1376827601) / (60 * 60 * 24)));
        $post_count = $user_info['posts'];
        $posts_created_count = mysql_fetch_row(mysql_query("SELECT COUNT(*) FROM {$db_prefix}topics WHERE id_member_started = {int:$user_id} AND id_board != 23"));
        ...
        $polls_made = mysql_query("SELECT COUNT(*)
        FROM {$db_prefix}topics
        WHERE id_member_started = {int:$user_id}
            AND id_board != 23
            AND id_poll != 0");
        $votes_made = mysql_query("SELECT COUNT(DISTINCT id_poll)
