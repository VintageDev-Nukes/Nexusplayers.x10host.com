			...
			if (!empty($board['children']))
			{
				...
				foreach ($board['children'] as $child)
				{
					if (!$child['is_redirect'])
						$child['link'] = '<a href="' . $child['href'] . '" ' . ($child['new'] ? 'class="new_posts" ' : '') . 'title="' . ($child['new'] ? $txt['new_posts'] : $txt['old_posts']) . ' (' . $txt['board_topics'] . ': ' . comma_format($child['topics']) . ', ' . $txt['posts'] . ': ' . comma_format($child['posts']) . ')">' . $child['name'] . ($child['new'] ? '</a> <a href="' . $scripturl . '?action=unread;board=' . $child['id'] . '" title="' . $txt['new_posts'] . ' (' . $txt['board_topics'] . ': ' . comma_format($child['topics']) . ', ' . $txt['posts'] . ': ' . comma_format($child['posts']) . ')"><img src="http://'.$_SERVER['SERVER_NAME'].'/Themes/default/images/' . str_replace("-utf8", "", $user_info['language']) . '/new.gif" class="new_posts" alt="" />' : '') . '</a>';
					else
	...
	<div class="tborder topic_table" id="messageindex">
		...
		foreach ($context['topics'] as $topic)
		{
			...
			if ($topic['new'] && $context['user']['is_logged'])
					echo '
							<a href="', $topic['new_href'], '" id="newicon' . $topic['first_post']['id'] . '"><img src="http://'.$_SERVER['SERVER_NAME'].'/Themes/default/images/' . str_replace("-utf8", "", $user_info['language']) . '/new.gif" alt="', $txt['new'], '" /></a>';
