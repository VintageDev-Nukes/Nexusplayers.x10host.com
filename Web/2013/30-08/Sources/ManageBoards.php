	...
	foreach ($cat_tree as $catid => $tree)
	{
		...
		foreach ($boardList[$catid] as $boardid)
		{
			$context['categories'][$catid]['boards'][$boardid] = array(
				...
				'image' => &$boards[$boardid]['img_filename'],
				'img_link' => &$boards[$boardid]['img_link'],
...
function EditBoard()
{
	...
	else
	{
		...
		$context['board']['image'] = htmlspecialchars($context['board']['image']);
		$context['board']['img_link'] = htmlspecialchars($context['board']['img_link']);
...
function EditBoard2()
{
	...
	if (isset($_POST['edit']) || isset($_POST['add']))
	{
		...
		$boardOptions['img_filename'] = preg_replace('~[&]([^;]{8}|[^;]{0,8}$)~', '&amp;$1', $_POST['img_filename']);
		$boardOptions['img_link'] = preg_replace('~[&]([^;]{8}|[^;]{0,8}$)~', '&amp;$1', $_POST['img_link']);
